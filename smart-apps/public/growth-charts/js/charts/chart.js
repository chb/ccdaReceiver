...
Chart.prototype = {
	...
	css : function( elem, selector )
	{
		if (!this.svgCss) {
			...
			var id = this.ID;
			$.each(this.svgCss, function( selector, style ) {
				$.each(style, function(k, v) {
					style[$.camelCase(k)] = v;
				});
				
				$.helperStyle(selector + "-" + id, style);
			});
		}
	...
	// @bottomBoundary
	_get_bottomBoundary : function( args ) 
	{
		var out    = null,
			inst   = this,
			lines  = [],
	...
	drawAxis : function()
	{
		...
		var bounds                   = this.get("dataBounds"),
			...
			q                        = 1,
			range                    = 0,
			i, y, x, r, node, step, val, precision, intersectTop, 
			intersectBottom;
		...
		if (range > 1) {
			step = Math.ceil(step);
			minY = Math.ceil(minY); 
			$.each(prefSteps, function(i, n) {
				if (i > 0 && step > prefSteps[i - 1] && step <= n) {
					step = n;
					minY = step;
					return false;
				}
