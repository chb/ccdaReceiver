(function(GC) {
	...
	PView.prototype = {
		...
		draw : function() {
			this.clear();
			this.paper.setSize(this.paper.canvas.clientWidth, this.paper.canvas.clientHeight);
		...
		_getVitals : function() {
			...
			$.each({
				...
				var lastEntry = getLastEnryHaving( meta.modelProp ), ds;
				if (lastEntry) {
					...
					if (ds) {
						out[key].percentile = GC.findPercentileFromX(
							out[key].value, 
							ds, 
							gender, 
							lastEntry.agemos
		...
		_setParentHeight : function(type, parentHeight, duration) {
			...
			img.attr("src", "img/pview/" + name + ".png");
		...
		_getHeuristics : function() {
			var out = {},
				...
				weightPctNow,
				weightPctPrev,
				bmi, healthyWeightMin, healthyWeightMax, D;
