...
.cleanupStep(Cleanup.simpleCode("2.16.840.1.113883.5.1119", "use"));
...
.cleanupStep(Cleanup.simpleCode("2.16.840.1.113883.5.45", "use"));
...
// supports specialized times like a medication's administration
// need it during initial cleanup phase, but then delete it
// when paring down the payload...
.cleanupDocs("xsitype", null)
.cleanupStep(function(){
...
.fields([
  ["sourceIds","1..*", "h:id", Identifier],
  ["dateRange", "1..1", "h:effectiveTime", EffectiveTime],
  ["concernStatus", "1..1", "h:statusCode/@code"],
...
.cleanupStep(Cleanup.simpleCode("2.16.840.1.113883.11.20.9.19", "concernStatus"));
...
.embed(ImmunizationInformation, "h:consumable/h:manufacturedProduct/")
.uriBuilder({
  category: "entries",
...
.fields([
  ...
  ["site","0..1", "h:approachSiteCode", ConceptDescriptor],
  ["administrationUnit","0..1", "h:administrationUnitCode", ConceptDescriptor],
  ["times", "0..2", "h:effectiveTime", EffectiveTime],
  ["dateRange", "0..1", "populated-in-cleanup", EffectiveTime],
  ["dosePeriod", "0..1", "populated-in-cleanup", PhysicalQuantity],
  ["freeTextSig","0..1", "h:text", TextWithReference],
  ["dose","0..1", "h:doseQuantity", PhysicalQuantity],
  ["rate","0..1", "h:rateQuantity", PhysicalQuantity],
])
.embed(MedicationInformation, "h:consumable/h:manufacturedProduct/")
.cleanupStep(function(){
...
.fields([
  ...
  ["gender",
   "1..1", 
   "h:patient/h:administrativeGenderCode", 
   SimplifiedCode, 
...
CCDA.prototype.run = function(node, callback){
  this.errors = [];
  this.super_.run.apply(this, arguments);
  return this;
};

function collectCodes(ccda){
  var allCodes = [];

  // validations?
  common.deepForEach(ccda, {
    pre: function(v){
      if (v instanceof Component){
        if (v instanceof ConceptDescriptor) {
          allCodes.push({data: v.js});
        }
        return v.js
      }
      return v;
...
module.exports.import = function(patientId, src, options, callback){
  ...
  ret.src = src;
  ret.run(xml, callback);

  // make each cleanup step run synchronously
  // but it may have async requirements to gather
  // before the step is run.  E.g. "validation'
  // cleanup steps may have a requirement that
  // relevant codes are pre-fetched from a database
  // and then decisions can be synchronously acted upon.

  var fetcher = require("../code-fetcher");
  if (options.skipValidation){
    fetcher = function(codes, cb){cb(null, []);};
  }
  var allCodes = collectCodes(ret);

  fetcher(allCodes, function(err, codes){
    ret.codes = codes;
