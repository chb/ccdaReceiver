...
Writer.prototype.write = function(callback){
  ...
  if (this.options.m) {
    tasks.dbInserts = [function(cb){dbInserts(self, cb);}];
  }
...
var remap = function(result, callback){
  ...
  function getExistingUris(k, done){
    ...
    db.patients.collection("links", function(err, links){
      ...
      links.find(q).toArray(function(err, matches){
        matches = matches || [];
...
function insertPatient(writer, cb){
	var d = writer.result.js.demographics;
  ...
  db.patients.collection("patients", function(err,collection ){
    ...
    doc.tokens = common.tokenizeDemographics(doc);
		if (writer.options.noIndex){
			doc.noIndex = true;
		}
    collection.update({"_id":doc._id}, doc, {upsert:true}, function(err){
...
function dbInserts(writer, callback){
