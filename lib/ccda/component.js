...
Component.register = function(){
  Component.registry[this.componentName] = this;
};
...
Component.embed = function(subc, relativePath) {
  var c = this;
  subc.parsers.forEach(function(p){
   var np = p;
   if(relativePath) {
    np = np.copy();
    np.xpath = relativePath + np.xpath;
   }
   c.parsers.push(np); 
  }); 
  return this;
...
Component.cleanupDocs = function(jsPath, fixedDocs){
    var np = null;
    if(fixedDocs) {
      np = new Parser();
      np.init.apply(np, fixedDocs);
    }
    this.extraDocs[jsPath] = np;
    return this;
};
